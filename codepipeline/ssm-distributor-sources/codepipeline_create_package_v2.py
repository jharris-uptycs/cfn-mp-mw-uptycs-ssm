...
AUTHFILE = ''
SECRET_STORE = os.environ['API_SECRET_ARN']
PACKAGE_DESCRIPTION = \
...
class UptApiAuth:
    ...
    def __init__(self, ssm_parameter=None, api_config_file=None, silent=True):
        # pylint: disable=too-many-arguments
        """Initialize a new UptApiAuth object.

        Args:
            api_config_file (str): Path to an API key file (default: None)
            ssm_parameter (str): The name of the SSM parameter to retrieve the API key from (default: None)
            silent (bool): Whether to suppress console output (default: True)
        """
        # if ssm_parameter is None and api_config_file is None:
        #     raise ValueError("Either an API key file or an SSM parameter name must be provided.")
            ...
            try:
                if not silent:
                    print(
                        ...
                        f'{api_config_file}')
                    self.get_json_from_auth_file(api_config_file)
            except FileNotFoundError as error:
                raise ApiConfigFileNotFoundError(
                    f"API config file not found: {error.filename}") from error

        else:
            self.get_json_from_secret(SECRET_STORE)

        try:
            key = self.authdata['key']
            secret = self.authdata['secret']
            domain = self.authdata['domain']
            customer_id = self.authdata['customerId']
        ...
        print(f'Got auth header {self.header}')

    def get_json_from_secret(self, secrets_name: str):
        """Retrieve a JSON object from an SSM parameter.

        Args:
            parameter_name (str): The name of the SSM parameter.
            with_decrypt (bool): Whether or not to decrypt the parameter value. Default is True.

        Returns:
            Dict: A dictionary containing the JSON object stored in the SSM parameter.
        """
        secret = SecretsManagerClient(secrets_name)
        self.authdata = json.loads(secret.get_secret_value())

    def get_json_from_ssm_parameter(self, parameter_name: str, with_decrypt: bool = True) -> Dict:
        """Retrieve a JSON object from an SSM parameter.
        ...
        self.authdata = json.loads(parameter_value)

    def get_json_from_auth_file(self, filename):
        with open(filename, encoding='utf-8') as file_handle:
            self.authdata = json.load(file_handle)
    ...
    def __init__(self, api_endpoint, method, payload=None, **kwargs):
        ...
        if method == 'GET':
            response = requests.get(self.api_auth.base_url + api_endpoint,
                                    ...
                                    timeout=TIMEOUT,
                                    **kwargs)
            print(f'Response to get Request {self.api_auth.base_url + api_endpoint}')
        elif method == 'POST':
...
class PackageDownloadsApi:
    ...
    def osquery_packages_get_version(self):
        ...
        value = response.response_json['items'][0]['version'].split('-')[0]
        print(f"package version {value}")
    ...
    def package_downloads_osquery_os_asset_group_id_get(self, os_name: str, dir_name: str,
        ...
        try:
            ...
            print(f'Calling API with {path}')
            self.logger.debug(f'Calling API with {path}')
            response = UptApiCall(path, 'GET')
            print(f'Got response for asset grp query{response.response_stream.status_code}')
            ...
            print('Response to Get resquest with path {path} response {response}')
